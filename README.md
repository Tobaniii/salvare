...
- Admin page: open `http://localhost:4123/admin` to view, add, update, or delete seeded domains, and to download a JSON export of coupons or result history. Backed by `GET/POST/DELETE /admin/coupons` and `GET /admin/export/{coupons,results}`. Read-only `POST /admin/import/preview/{coupons,results}` endpoints validate exported JSON without applying changes; actual import remains CLI-only via `npm run db:import`.
