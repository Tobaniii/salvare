...
- **v0.51.0** — Test-order-only **pre-rank signal layer** (winner selection, response shape, and schema unchanged). The v0.38 pre-order now reads `coupon_code_sources.expires_at` (pre-order path only) and folds it into the existing merged per-code quality score as a **deprioritize-only tier**: expired codes (every dated claim past, `Math.max` over claim expiries, strict `<` against the injected clock) sort to the end of the test queue but stay in the set and are still tested — never dropped or filtered (§9). null/absent/garbage `expiresAt` is not expiry and carries no penalty. Cross-provider claims still collapse into one internal score; no `coupon_results`/history read is added to the pre-order layer. A new pure [`server/merchant-alias.ts`](server/merchant-alias.ts) resolves a curated, explicit, exact-key set of merchant domains to one canonical store key after `normalizeLookupDomain` — an in-code constant (no DB table, schema stays `"5"`) **shipped EMPTY** so it is the identity function and `/coupons` is byte-identical to v0.50; zero cross-merchant bleed. `expires_at`/`expiresAt` never appears in any response/provenance/error; `rankCandidateCodes`, `CouponApiResponse`, export/import shapes, and extension behavior are unchanged; the winner is still the lowest verified `finalTotalCents` (`savingsCents` tiebreak, source confidence only when totals and savings are exactly equal). Server-only; no extension TS or bundle change.
- **v0.52.0** — Opt-in **scheduled source-refresh** loop ([`server/scheduled-refresh.ts`](server/scheduled-refresh.ts)), first consumer of `activation.schedulerSupported`. A **default-OFF, fail-closed** background loop periodically re-runs the existing manual import path (`runSourceRefresh` → idempotent `importProviderCandidates`) so the candidate pool stays warm without a manual CLI run — **not a ranking input** (winner still lowest verified `finalTotalCents`; §7/§9 firewall untouched). One env toggle `SALVARE_SCHEDULED_REFRESH_ENABLED` is the kill switch; `SALVARE_SCHEDULED_REFRESH_INTERVAL_MS` (default/floor 6h = Awin cache TTL) sets the cadence. Every tick goes through the **same `canFetchSourceNow`** cache/rate gate (silent skip on `cache_fresh`/`recent_attempt`/`unknown_source`); domains come from existing provenance (`stores` ⋈ `coupon_code_sources`); sequential, single-flight, error-isolated. Awin flips `schedulerSupported:true`; **impact.com stays permanently `false`** (never scheduled). Non-blocking start after `server.listen()` with the first graceful SIGINT/SIGTERM handler. No schema bump (`"5"`), no new tables, no `/coupons`/`/results`/extension/bundle change; injectable clock + timer + fetcher (no live HTTP / real timers in tests).
...
- **v0.49.0** — impact.com adapter **real-shaped** (gated, not live). The v0.42 placeholder `Bearer` header is replaced by the *documented* impact.com **HTTP Basic** scheme (`Authorization: Basic base64(accountSid:authToken)`), built only from already-read config (never client input) via a new optional `buildAuthHeader` spec hook on the shared pipeline ([`server/source-provider-pipeline.ts`](server/source-provider-pipeline.ts)); Awin omits the hook and stays **byte-identical** (`Bearer`). `SALVARE_IMPACT_ACCOUNT_SID` is now **required, fail-closed** (blank/missing → disabled, reason `missing_account_sid`) — it is in both the URL path and the Basic pair — with a sibling `preflight` hook failing closed before any fetch/cache-read. The adapter speaks the documented endpoint/envelope/field shape; un-live-verifiable assumptions (auth = HTTP Basic, envelope/field-name case, **pagination — single-batch fetch only**) are marked "documented, not live-verified" in code and added as explicit **OPEN** items to the §4 checklist in [`docs/SOURCE_PROVIDER_RESEARCH.md`](docs/SOURCE_PROVIDER_RESEARCH.md) (no box checked — needs human ToS/credential review). A new `impact-offers-realistic-contract.json` fixture + a dedicated **HTTP Basic credential redaction** test prove the header value, the raw base64 token, and the decoded `accountSid:authToken` pair never reach the result, fetch log, cache, status summary, or any error. New cross-provider tests prove Awin + Impact candidates for the same domain dedupe correctly, preserve per-provider `coupon_code_sources` provenance, and record `import_history.provider_id` per provider — Impact via the **internal** path only. Impact stays `userExposed:false`/`importEnabled:false` (`resolveProvider` still denies it `not_user_exposed` on preview AND import, unchanged from v0.48); no live HTTP, no schema bump (stays `"5"`), no new providers, no scraping, no UI; the live flip is a separate owner-gated follow-up.
- **v0.48.0** — Provider **activation framework**. The split registry gating — `ProviderCapabilities` (`preview`/`importSupported`/`cacheSupported`) plus the sibling `descriptor.userExposed` — is unified into one `ProviderActivation` shape ([`server/source-provider-registry.ts`](server/source-provider-registry.ts)) with exactly six fields (`enabled`, `previewEnabled`, `importEnabled`, `userExposed`, `cacheSupported`, `schedulerSupported`; `preview → previewEnabled`, `importSupported → importEnabled`). A master `enabled` gate is enforced fail-closed in `resolveProvider` via a new pure exported `classifyActivation(activation|null, purpose)`: precedence `unknown_provider` > `provider_disabled` > `not_user_exposed` > `capability_unsupported` (new `provider_disabled` added to `ResolveDenyReason`, propagates to the preview/import routes automatically). Both providers ship `enabled:true` (Awin all-on + exposed; Impact preview+cache only, hidden) so behavior is **byte-identical to v0.47** — the disabled path is test-double only. `schedulerSupported` is declared-only (false, no consumer/enforcement, v0.52). Flags stay compile-time constants — no env/DB/runtime toggling, no persistence (schema stays `"5"`). `GET /admin/source-providers` now carries a nested `activation` 5-field subset (`userExposed` still the filter gate, never echoed) + a new **read-only** "Provider activation" admin.html section (no toggles; hidden providers stay absent). No route-handler logic changed beyond the projection; Impact stays hidden (v0.49).
- **v0.47.0** — Generic provider **pipeline execution layer**. The structurally-identical Awin (v0.32/v0.33) and Impact (v0.42) adapters become thin spec builders that delegate one shared `runProviderPipeline` ([`server/source-provider-pipeline.ts`](server/source-provider-pipeline.ts)), extracted verbatim from the live Awin adapter (config/key/domain gate → cache-read short-circuit *only when* `spec.cacheSupported` → fetch → HTTP/parse/envelope error mapping → shared `buildCandidate` iteration → one `recordSourceFetchAttempt` per fail/success path → `upsertSourceCacheEntry` once on success). Provider divergence is isolated to the injected spec (config gate, URL shaping, response-envelope key, per-row deny/normalize via `mapRow`). **Impact reaches internal capability parity** — same cache-read short-circuit (`cacheSupported: false → true`) and a `cacheHit` result field — while staying registry-internal (`importSupported:false`/`userExposed:false` unchanged; `resolveProvider` still denies Impact on preview+import). Error-code union unified into one shared `ProviderAdapterErrorCode` (type-only widening); provider health derivation folded into one shared registry helper. Awin output is byte-identical to v0.46, proven by characterization pins captured pre-refactor (cache-fresh-hit, cache-miss full fetch, `disabled`/`missing_api_key`/`parse_error` early returns) that stay green unchanged through the rewrite. No schema bump (stays `"5"`), no route-handler changes, no new providers, no scraping, no live HTTP in tests.
