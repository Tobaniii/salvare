...
## 9g. v0.37.0 — admin-visible source/provenance summary

No DB schema change. v0.37.0 lands the first **reader** that aggregates `coupon_sources` and `coupon_code_sources` for admin UI consumption: [`server/db-source-summary.ts`](../server/db-source-summary.ts) exposes `getSourceSummaryForDomain(db, domain)` which executes SELECT-only joins and returns a bounded (`SOURCE_SUMMARY_CODE_CAP = 500`) shape `{ domain, storeId, codeCount, sourceCount, truncated, codes[], sourceSummary[] }`. The helper writes nothing — `coupon_codes`, `coupon_code_sources`, `coupon_results`, `source_cache`, `source_fetch_log`, and `coupon_sources` are all untouched by this read path. `sourceUrl` is deliberately not surfaced; only `sourceId`, `sourceName`, `sourceType`, `discoveredAt`, `label`, `expiresAt`, and `confidence` are returned per code claim, plus `sourceId`/`sourceName`/`sourceType`/`codeCount` per source. Public API response shapes (`/coupons`, `/admin/coupons`, `/admin/coupon-stats`, `/admin/export/*`, `/admin/import/*`, `/results`, `/health`), export/import JSON shapes, ranking, and extension behavior are **unchanged**.
